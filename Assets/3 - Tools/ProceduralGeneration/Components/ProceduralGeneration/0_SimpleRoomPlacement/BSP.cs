...
public class BSP : ProceduralGenerationMethod
{
    ...
    private async UniTask GenerateRoomsAsync(BSPNode node, CancellationToken cancellationToken)
    {
        if (node.IsLeaf)
        {
            int maxSize = Mathf.Min(node.Area.width, node.Area.height) - 2;
            int roomSize = RandomService.Range(_minRoomSize.x, Mathf.Max(_minRoomSize.x + 1, maxSize));

            // Centre de la zone BSP
            int centerX = node.Area.x + node.Area.width / 2;
            int centerY = node.Area.y + node.Area.height / 2;

            // Crée une salle carrée centrée dans la zone
            int roomX = centerX - roomSize / 2;
            int roomY = centerY - roomSize / 2;

            RectInt room = new RectInt(roomX, roomY, roomSize, roomSize);
            node.Room = room;
            _rooms.Add(room);
    ...
    private async UniTask ConnectRoomsAsync(BSPNode node, CancellationToken cancellationToken)
    {
        ...
        a = GetRoomCenter(node.Left);
        b = GetRoomCenter(node.Right);
    ...
    {
        Vector2Int a = GetRoomCenter(nodeA);
        Vector2Int b = GetRoomCenter(nodeB);
        return (a, b);
    }
